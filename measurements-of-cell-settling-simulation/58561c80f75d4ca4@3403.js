// https://observablehq.com/@csiz/cell-settling@3403
...
`
  ...
  main.variable(observer()).define(["d3","DOM","setup_display","env_size","radio","slider","equilibrium_area","polarization_factor","run_loop","init_physics","polarized_initial_cells","visibility","periodic_boundaries","lennard_jones_max_distance","paint_cells","paint_actins","paint_polarization","details_to_show","pretty_float","paint_details","apply_membrane_forces","apply_membrane_stiffness","apply_nucleus_forces","apply_intercell_forces","advance_physics","tick_duration","invalidation","record_canvas"], function(d3,DOM,setup_display,env_size,radio,slider,equilibrium_area,polarization_factor,run_loop,init_physics,polarized_initial_cells,visibility,periodic_boundaries,lennard_jones_max_distance,paint_cells,paint_actins,paint_polarization,details_to_show,pretty_float,paint_details,apply_membrane_forces,apply_membrane_stiffness,apply_nucleus_forces,apply_intercell_forces,advance_physics,tick_duration,invalidation,record_canvas)
{
  ...
  let loop = run_loop({
    ...
    update: async function({cells, time, phase}, {fps}) {
      ...
      // Paint info.
      let extra_lines = [];
      if (compression !== undefined && details_to_show.some(o => o === "compression")) extra_lines.push(`compression = ${pretty_float(compression)}`);
      if (sim_polarization_factor !== undefined && details_to_show.some(o => o === "polarization")) extra_lines.push(`polarization = ${pretty_float(sim_polarization_factor)}`);
...
lennard_jones_distance * 1.3
  ...
  main.variable(observer("membrane_polarization")).define("membrane_polarization", ["md","tex"], function(md,tex){return(
...
function generate_random_cell_centers({env_size, border, min_nucleus_separation, n_cells, random=Math.random}){
  let points = [];
  
  let random_x = d3.randomUniform.source(random)(border.width, env_size.width - border.width);
  let random_y = d3.randomUniform.source(random)(border.height, env_size.height - border.height);
...
function generate_random_polarization(cells, random=Math.random){
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`Because we shrink the cell polygons we need a minium separation between the initial cell positions, and between cells and the border.`
)});
...
function paint_forces(cells, forces_to_show, display, options=forces_options) {

  let {equiv_duration} = options;
...
function paint_details({time, fps, extra_lines=[]}, display, transform=undefined) {
  ...
  let lines = [`time = ${pretty_float(time)}s`, ...extra_lines];
  ...
  main.variable(observer("forces_options")).define("forces_options", function(){return(
{
  ...
  // The force arrows represent movement over the equivalent duration: 
  equiv_duration: 8,
}
)});
